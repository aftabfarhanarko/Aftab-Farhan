...
> = {
  ...
  default: {
    iconWrap: "bg-foreground/[0.07] border-white/[0.08]",
  ...
  return (
    <div className="grid gap-4 sm:gap-5 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
      {Array.from({ length: 6 }).map((_, i) => (
        <div
          key={i}
          className="p-5 rounded-2xl border border-white/[0.07] bg-foreground/[0.02] animate-pulse"
        >
        ...
        <div className="grid gap-4 sm:gap-5 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
          {categories?.map((category) => {
            ...
            return (
              <div
                key={category.id}
                className="group flex flex-col p-5 sm:p-6 rounded-2xl border border-white/[0.07] bg-foreground/[0.02] hover:border-white/[0.15] hover:bg-foreground/[0.04] transition-all duration-300"
              >
                {/* Card header */}
                <div className="flex items-center gap-3 mb-4">
                  <div
                ...
                <div className="flex flex-wrap gap-1.5">
                  {category.skills.map((skill) => (
                    <div
                      key={skill.id}
                      className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border border-white/[0.07] bg-white/[0.03] hover:border-white/[0.18] hover:bg-white/[0.06] transition-all duration-200 group/chip cursor-default"
                    >
                      {/* Logo or accent dot */}
                      {skill.imageUrl ? (
