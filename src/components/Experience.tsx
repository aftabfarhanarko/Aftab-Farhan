  ...
  return (
    <section id="experience" className="mb-32 scroll-mt-24">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div className="p-4 rounded-xl bg-gradient-to-br from-accent-muted/10 to-transparent border border-accent-muted/10 text-center">
      ...
      <div className="relative">
        {/* Vertical Line */}
        <div className="absolute left-8 top-0 bottom-0 w-px bg-gradient-to-b from-accent/30 via-accent/20 to-transparent hidden md:block" />
        ...
        <div className="space-y-8">
          {filteredExperiences.map((exp, index) => (
            <div key={exp.id} className="relative">
              {/* Timeline Dot */}
              <div
                className="absolute left-8 -translate-x-1/2 w-4 h-4 rounded-full bg-accent border-4 border-background hidden md:block"
                style={{ top: "2rem" }}
              />
              ...
              <div className="md:ml-16">
                ...
                <div
                  className={`p-6 rounded-2xl bg-gradient-to-br ${
                    exp.type === "current"
                      ? "from-accent/10 via-accent/5 to-transparent border-accent/30"
                      : "from-accent-muted/10 to-transparent border-accent-muted/20"
                  } border mb-4`}
                >
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                    ...
                    <div className="flex items-center gap-3">
                      {exp.type === "current" && (
                        <span className="px-3 py-1.5 bg-green-500/10 text-green-500 rounded-full text-xs font-medium flex items-center gap-1.5">
                          ...
                          <div
                            className={`p-1.5 rounded-md ${
                              exp.type === "current"
                                ...
                                : "bg-accent-muted/10 text-foreground/60"
                            }`}
                          >
                            {getIcon(role.icon)}
                          </div>
                        ...
                        <ul className="grid md:grid-cols-2 gap-x-4 gap-y-1.5 ml-11">
                          {role.responsibilities.map((item, i) => (
                            <li key={i} className="flex items-start gap-2">
                              <svg
                                className="w-3.5 h-3.5 text-accent mt-1 flex-shrink-0"
                                fill="none"
                                viewBox="0 0 24 24"
                                stroke="currentColor"
                              >
                  ...
                  {/* Tech Stack */}
                  <div className="mt-4 pt-4 border-t border-accent-muted/10">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {exp.techStack.map((tech) => (
                        <span
                          key={tech}
                          className="px-2.5 py-1 text-xs bg-accent-muted/5 border border-accent-muted/10 rounded-md text-foreground/60"
                        >
                          {tech}
                        </span>
                      ))}
      ...
      <div className="mt-8 p-6 rounded-xl bg-accent-muted/5 border border-accent-muted/10">
        ...
        <div className="flex flex-wrap gap-2">
          {companyServices.map((service) => (
            <span
              key={service}
              className="px-3 py-1.5 text-sm bg-accent-muted/5 border border-accent-muted/10 rounded-full text-foreground/70 hover:text-foreground hover:border-accent/30 transition-colors"
            >
              {service}
            </span>
          ))}
